...
from model.db_user_services import set_status_chat, delete_status_chat
...
from telegram_service.messages import send_approve_keyboard, remove_message, send_message_user
...
async def on_start_bot(message: dict, redis: redis.Redis = Depends(get_redis)):
    ...
    await send_approve_keyboard(target_chat_id=chat_id, message_id=message_id, from_channel=chat_id)
...
async def on_click_button(message: dict):
    ...
    if complete_operation['succ']:
        ...
        await send_message_user(chat_id=chat_id, text='Обратиться к менеджеру вы сможете в следующих версиях )')
    async with httpx.AsyncClient() as client:
        url = settings.API_URL + '/api/registration/' + settings.WEBHOOK_SECRET_KEY
        user_data = UserCreateSchema(id=user_id, user_name=user_name, tg_channel_id=chat_id, status_chat=status_chat)
