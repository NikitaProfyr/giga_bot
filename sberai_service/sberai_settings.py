    ...
    async def authorization(self):
        ...
        try:
            async with httpx.AsyncClient(verify=False) as cl:
                data = await cl.post(url=self.base_url + '/oauth', headers=self.__headers, data=f'scope=GIGACHAT_API_PERS')
                data = data.json()
...
ai_client = AIClient(base_url=url, secret_key=SBER_SECRET_API_KEY, auth_data=SBER_AUTHORIZATION_DATA)
